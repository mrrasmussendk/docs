...
## Sync docs from another repository

You can pull docs from another repository in a GitHub Action, commit the synced files in this docs repository, and let that push trigger a new Mintlify deployment.

To trigger the sync when the source repository changes, send a `repository_dispatch` event from the source repository to this docs repository and run a sync workflow here.

```yaml
name: Sync docs from source repo
on:
  repository_dispatch:
    types: [sync-docs]
  workflow_dispatch:
    ...
    steps:
      ...
      - name: Checkout source repository
        ...
        with:
          ...
          path: source
      - run: rsync -av --delete source/docs/ ./imported-docs/
      - run: |
          git config user.name "github-actions[bot]"
          git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
          git add imported-docs
          git commit -m "chore: sync docs from source repo" || exit 0
          git push
```

If the source repository is private, configure a token with read access to `your-org/source-repo` in the checkout step.
The `--delete` flag removes files in `./imported-docs/` that are no longer present in `source/docs/`.
The `git commit ... || exit 0` line keeps the job green when there are no sync changes to commit.
